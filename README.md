## 👋 Hi, I'm Aradhya Tagotra

🎓 Final-year B.Tech Computer Science student  
💻 Aspiring Full Stack Developer  
🌐 Passionate about building scalable, user-friendly web applications  

---

## 🚀 About Me

I am a Computer Science undergraduate with hands-on experience in full stack web development.  
I enjoy building responsive web applications using modern frontend and backend technologies and continuously improving my skills through projects and internships.

---

## 🛠️ Technical Skills

**Frontend:** HTML, CSS, JavaScript, React  
**Backend:** Node.js, Express.js, MongoDB, SQL  
**Programming:** Java (Basics), Python  
**Tools:** Power BI (Basic), MS Excel  

---

## 📂 Projects

### 🔹 SIGMA GPT – AI Chatbot
A full-stack AI chatbot inspired by ChatGPT.
- Frontend: HTML, CSS, JavaScript, React
- Backend: Node.js, Express.js
- Focused on responsive UI and user interaction

🔗 Live Demo: https://github.com/31820709/SIGMA-GPT/tree/main

---

### 🔹 Weather App – React
A dynamic weather application built using React components.
- Real-time weather data
- Location-based search
- Responsive design

🔗 Live Demo: https://github.com/31820709/Weather-App-

---

### 🔹 Spotify Clone – Frontend Project
A Spotify-inspired UI clone built using HTML and CSS.
- Clean and modern UI
- Responsive layout
- Focus on user experience

🔗 Live Demo: https://github.com/31820709/Spotify-Clone

---

## 📜 Certifications

- Internship in Machine Learning – Livewire  
- Internship in Cyber Security – Corizo  
- NPTEL Certification – Ethical Hacking and Cyber Security 

---

## 📫 Contact Me

📧 Email: aradhya.ycet@gmail.com 

---

⭐ *Thank you for visiting my GitHub profile!*
